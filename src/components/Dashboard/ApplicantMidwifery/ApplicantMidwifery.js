...
const ApplicantMidwifery = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axiosPublic.get("/computerOperator");
    ...
    console.log("allapplicants data computer operators", applicantsData);
                    ...
                    <tbody>
                        {applicantsData.map((item, index) => (
                            <tr key={item._id} className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
                                ...
                                <td className="px-6 py-4 text-center border border-gray-300">{item?.p_number || "N/A"}</td>
...
export default ApplicantMidwifery;